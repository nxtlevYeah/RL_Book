...
from agents.a2c.a2c_network import A2CNetwork
...
class A2CLearner(Learner):
    ...
    def __init__(self,
        ...
        """
            ...
            정책과 가치 함수를 학습하기 위한 Adam 옵티마이저, 학습률 스케쥴러, 입실론 스케쥴러를 생성한다.
        Args:
            config: 설정
            logger: 로거
    ...
    def _loss(self,
        ...
        """
            A2C 알고리즘의 목적 함수에 따라 정책의 손실을 계산 하고,
        ...
        # 2. 정책 손실 계산
        policy_loss = 0     # your code

        # 3. 가치 함수 손실 계산
        value_loss = 0      # your code

        # 4. 엔트로피 보너스 계산
        entropy_loss = 0    # your code
    ...
    def update(self, total_n_timesteps: int, total_n_episodes: int) -> bool:
        ...
        for epoch in range(0, self.config.n_epochs):
            for i in range(num_batch_times):
                ...
                # 8. 손실 계산
                total_loss, loss_results = \
                    self._loss(state, action, target_value, advantage)

                # 9. 백워드 패스 실행 (그레이디언트 계산)
