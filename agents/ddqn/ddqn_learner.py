...
from agents.ddqn.ddqn_network import DDQNNetwork
...
class DDQNLearner(DQNLearner):
    ...
    def __init__(self,
                 ...
                 buffer: ReplayBuffer):
        """
            DQNLearner 클래스의 초기화 메서드를 호출하여 학습자를 초기화
        Args:
            config: 설정
            logger: 로거
    ...
    def update(self, total_n_timesteps: int, total_n_episodes: int) -> bool:
        """
            더블 Q-러닝 업데이트 식에 따라 타깃을 계산해서 휴버 손실로 가치 함수를 학습
            그리고, 타깃 가치 함수 모델을 하드/소프트 업데이트하고 성능 정보를 로깅
        ...
        for i in range(self.config.gradient_steps):
            ...
            with torch.no_grad():
                ...
                target_q_value = reward + \
                                 (1-done)*self.config.gamma*next_max_q_value
